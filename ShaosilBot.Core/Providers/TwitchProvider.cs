...
namespace ShaosilBot.Core.Providers
{
	public class TwitchProvider : ITwitchProvider
	{
		...
		public async Task HandleNotification(TwitchPayload payload)
		{
			...
			lastMessage = messages.FirstOrDefault(m => m.Author.Username == "ShaosilBot" && m.Embeds.Count > 0 && new Regex($"^(🔴 \\[LIVE\\] )?{payload.event_type.broadcaster_user_name} ").IsMatch(m.Embeds.First().Title)) as RestUserMessage;
