...
namespace ShaosilBot.Providers
{
    public class TwitchProvider
    {
        ...
        {
            var oauthInfo = JsonSerializer.Deserialize<OAuthInfo>(await _blobProvider.GetBlobTextAsync(OAuthFileName, true));
            if (oauthInfo.Expires.UtcDateTime.AddSeconds(-10) < DateTime.UtcNow)
            {
                ...
                await _blobProvider.SaveBlobTextAsync(OAuthFileName, JsonSerializer.Serialize(oauthInfo));
            }
            else
                _blobProvider.ReleaseFileLease(OAuthFileName);
